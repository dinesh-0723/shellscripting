...
NODEJS() {
  ...
  sed -i -e 's/MONGO_DNSNAME/monodsh.roboshop.internal/' -e 's/REDIS_ENDPOINT/newredis.roboshop.internal/' -e 's/MONGO_ENDPOINT/monodsh.roboshop.internal/' -e 's/CATALOGUE_ENDPOINT/catsh.roboshop.internal/' /home/roboshop/${COMPONENT}/systemd.service &>>${LOG_FILE} && mv /home/roboshop/${COMPONENT}/systemd.service /etc/systemd/system/${COMPONENT}.service &>>${LOG_FILE}
  statcheck $?
